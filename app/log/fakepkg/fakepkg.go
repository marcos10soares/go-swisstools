...
func NestedErrorOneLevel() error {
	...
	if errors.Cause(err).Error() == ErrLevel2ErrorMsg {
		...
		// return errors.WithMessage(err, ErrLevel1ErrorMsg) // this makes ErrLevel2Error identifiable in main, but with no stack trace

		return errors.Wrap(err, ErrLevel1ErrorMsg) // this makes ErrLevel1Error identifiable in main, with stack trace to this line

		// return errors.Wrap(errors.New(ErrLevel1ErrorMsg), err.Error()) // this makes ErrLevel1Error identifiable in main, with stack trace to this line
